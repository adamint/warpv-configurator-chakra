...
export function PipelineSettingsForm({ pipelineSettings, setPipelineSettings, formErrors }) {

  function handleValueUpdate(key, type, value) {
    const newObj = { ...pipelineSettings };
    newObj[key] = value;
    setPipelineSettings(newObj);
  }
    ...
    {ConfigurationParameters.map(configurationParameter => {
      ...
      if (configurationParameter.type === Int) {
        return <FormControl mb={3} key={configurationParameter.jsonKey}
                            isInvalid={configurationParameter.validator && pipelineSettings[jsonKey] !== undefined
                            && pipelineSettings[jsonKey] && !configurationParameter.validator(pipelineSettings[jsonKey], configurationParameter)}>
        ...
        return <FormControl mb={3} key={configurationParameter.jsonKey}>
          ...
          <RadioGroup
            ...
            value={pipelineSettings[configurationParameter.jsonKey] || 'None'} defaultValue='None'>
            <Stack direction='row'>
